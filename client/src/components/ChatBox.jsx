...
import { clearChat } from '../App/feature/chatSlice';
...
const ChatBox = () => {
    ...
    const handleSendMessage = async (e) => {
        ...
        setMessages([...messages, {
            id: Date.now(), chatId: chat.id, sender_id: user.id, message: newMessage, 
            createdAt: new Date()
        }]);
        setNewMessage("")
    }
    ...
    return (
        <div className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex items-center justify-center sm:p-4'>
            ...
            <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-[500px] flex flex-col'>
                    ...
                    {isLoading ? (
                        ...
                        messages.map((message) => (
                            <div key={message.id} className={`flex ${message.sender_id ===
                                ...
                                <div className={`max-w-[70%] rounded-lg p-3 pb-1 ${message.
                                    sender_id === user.id ? "bg-indigo-600 text-white" :
                                    ...
                                    <p className={`text-[10px] mt-1 ${message.sender_id ===
                                        user.id ? "text-indigo-200" : "text-gray-400"}`}>
                    ...
                    : (
                        <div className="p-4 bg-white border-t border-gray-200 rounded-b-lg">
                            <p className='text-sm text-gray-600 text-center'>
                                {chat ? `Listing is ${chat?.listing?.status}` : "Loading chat..."}</p>
                        </div>
                    )
                }
            </div>
