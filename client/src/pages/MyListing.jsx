...
const MyListing = ({ onClose }) => {
  ...
  const totalValue = userListings?.reduce((sum, listing) => sum + (listing.price || 0), 0) || 0;
  ...
  return (
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 pt-8'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-8'>
        <StatCard title='Total Listings' value={userListings?.length || 0} icon={<Eye
          className='size-6 text-indigo-600' />} color='indigo' />

        <StatCard title='Active Listings' value={activeListings} icon={<CheckCircle
          className='size-6 text-green-600' />} color='green' />

        <StatCard title='Sold' value={soldListings} icon={<TrendingUp
          className='size-6 text-indigo-600' />} color='indigo' />
